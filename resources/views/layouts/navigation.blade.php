        ...
        @if(Auth::user()->isAdmin())
            ...
            class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 group
                    {{ request()->routeIs('configuracion.*')
                        ? 'bg-teal-400/20 text-teal-500 shadow-sm'
                        : 'text-white/60 hover:bg-white/8 hover:text-white' }}">

                <span class="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg transition-colors
                            {{ request()->routeIs('configuracion.*') ? 'bg-teal-400/20' : 'bg-white/5 group-hover:bg-white/10' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @if(request()->routeIs('configuracion.*'))
                    <span class="ml-auto w-1.5 h-1.5 bg-teal-500 rounded-full"></span>
                @endif
            </a>
        @endif
    </nav>
