...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .portada-titulo-bloque h1.grande { font-size: 22pt; }
...
<div class="portada">
    ...
    <div class="portada-contenido">
        <div class="portada-titulo-bloque">
            ...
            <br>
            <h1>{{ $registro->empresa_nombre ?? '' }}</h1>
            <br>
            <h1 class="mediano">{{ $formulario->lugar_muestreo ?? '' }}</h1>
            <br>
            @if($registro->mes_anio_informe ?? null)
                <h1 class="mediano">{{ \Carbon\Carbon::parse($registro->mes_anio_informe)->locale('es')->isoFormat('MMMM YYYY') }}</h1>
                <br>
            @endif
...
<div class="pagina">
    ...
    <div class="page-content">
        ...
        <table>
            <tr><td colspan="4" class="th-seccion">2. INFORMACIÓN DE MUESTREO</td></tr>
            <tr>
                <td class="td-label" colspan="2">Lugar de Muestreo</td>
                <td class="" colspan="2">{{ $formulario->lugar_muestreo ?? '' }}</td>
            </tr>
            <tr>
                <td class="td-label" colspan="2">Dirección de Muestreo</td>
                <td class="" colspan="2">{{ $formulario->direccion_muestreo ?? '' }}</td>
            </tr>
            <tr>
                <td class="td-label" colspan="2">Identificación Punto de Muestreo</td>
                <td class="" colspan="2">{{ $formulario->punto_muestreo ?? '' }}</td>
            </tr>
            <tr>
                <td class="td-label" style="width:25%">Fecha y Hora Inicio Muestreo</td>
                <td class="" style="width:25%">
                    {{ $formulario->inicio_muestreo ? \Carbon\Carbon::parse($formulario->inicio_muestreo)->format('d/m/Y') : '' }}
                </td>
                <td class="td-normal" style="width:25%; text-align:center">
            ...
            <tr>
                ...
                <td class="td-normal" style="text-align:center">
                    {{ $formulario->fin_muestreo ? \Carbon\Carbon::parse($formulario->fin_muestreo)->format('H:i') : '' }}
                </td>
                <td></td>
            </tr>
            <tr>
                <td class="td-label" colspan="2">Tipo de Muestra</td>
                <td class="" colspan="2">Muestreo puntual compuesto</td>
            </tr>
        </table>
        ...
        <table>
            ...
            <tr>
                ...
                <td style="text-align:center; font-weight:bold; color:{{ $eq['chk'] ? '#006600' : '#333' }}">
                    {{ $eq['chk'] ? 'SI' : '—' }}
                </td>
            </tr>
            @endforeach
...
══════════════════════════════════════════════════════ --}}
@foreach([
    ['file' => $formulario->anexo_2_file ?? null, 'titulo' => $formulario->anexo_2_titulo ?? 'Documentación Técnica',  'n' => 2, 'pag' => 4],
    ['file' => $formulario->anexo_3_file ?? null, 'titulo' => $formulario->anexo_3_titulo ?? 'Cadena de Custodia',     'n' => 3, 'pag' => 5],
    ['file' => $formulario->anexo_4_file ?? null, 'titulo' => $formulario->anexo_4_titulo ?? 'Otros',                  'n' => 4, 'pag' => 6],
] as $anexo)
    @if($anexo['file'])
    <div class="pagina">
        ...
        <div class="footer-interno">
            ...
            <div class="footer-pagina">
                Página <span class="page-number"></span> / <span class="total-pages"></span>
            </div>
            --}}
        </div>
    </div>
