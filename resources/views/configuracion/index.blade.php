<x-app-layout>
    ...
    <div class="p-4 sm:p-6 lg:p-8 space-y-8">
        ...
        <div x-data="equiposManager()" x-init="init()" class="space-y-5">
            ...
            --}}
            {{-- Agregar equipo --}}
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden" x-data="{ abierto: {{ $errors->has('codigo') ? 'true' : 'false' }} }">
                <button type="button" @click="abierto = !abierto"
                    ...
                    <svg class="w-4 h-4 text-gray-400 transition-transform duration-200"
                        :class="abierto && 'rotate-180'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-show="abierto" x-transition:enter="transition ease-out duration-200"
                    ...
                    <form method="POST" action="{{ route('configuracion.equipos.store') }}" class="px-5 py-4">
                        ...
                        {{-- Fila 2: Modelos (checkboxes) + Botón --}}
                        <div class="flex flex-col sm:flex-row gap-3 items-end">
                            <div class="flex-1" x-data="{ busqueda: '', modelos: @js($modelos->map(fn($m) => ['id' => $m->id, 'nombre' => $m->nombre])->values()) }">
                                <label class="block text-xs font-medium text-gray-500 mb-1.5">
                                    Modelos asociados <span class="text-gray-400">(opcional, selección múltiple)</span>
                                </label>
                                <div
                                    class="rounded-xl border border-gray-200 bg-gray-50 overflow-hidden transition-all focus-within:border-orange focus-within:shadow-[0_0_0_3px_rgba(255,140,66,0.15)]">
                                    {{-- Buscador --}}
                                    <div class="flex items-center gap-2 px-3 py-2 border-b border-gray-200 bg-white">
                                        ...
                                        <input type="text" x-model="busqueda" placeholder="Buscar modelo..."
                                            class="w-full bg-transparent border-none p-0 text-xs text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-0">
                                    </div>
                                    {{-- Lista checkboxes --}}
                                    <div class="max-h-32 overflow-y-auto py-1">
                                        <template
                                            x-for="mod in modelos.filter(m => m.nombre.toLowerCase().includes(busqueda.toLowerCase()))"
                                            :key="mod.id">
                                            <label
                                                class="flex items-center gap-2.5 px-3 py-1.5 hover:bg-orange/5 cursor-pointer transition-colors">
                                                <input type="checkbox" :name="'modelos_ids[]'" :value="mod.id"
                                                    class="w-3.5 h-3.5 rounded border-gray-300 text-orange focus:ring-orange cursor-pointer">
                                                <span class="text-xs text-gray-700" x-text="mod.nombre"></span>
                                            </label>
                                        </template>
                                        <p x-show="modelos.filter(m => m.nombre.toLowerCase().includes(busqueda.toLowerCase())).length === 0"
                                            class="px-3 py-2 text-xs text-gray-400 text-center italic">Sin resultados
                                        </p>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div
                    class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-gray-100 bg-gray-50/50">
                    <div class="flex items-center gap-2.5">
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        <div
                            class="flex items-center rounded-xl border border-gray-200 bg-white overflow-hidden divide-x divide-gray-200 text-xs font-medium">
                        ...
                        <div
                            class="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2 w-full sm:w-52
                    ...
                    <table class="w-full text-sm table-fixed">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse($equipos as $equipo)
                                <tr class="equipo-fila hover:bg-gray-50/70 transition-colors"
                                    x-show="filtraFila('{{ strtolower($equipo->codigo) }}', '{{ strtolower($equipo->descripcion ?? '') }}', {{ $equipo->activo ? 'true' : 'false' }}, $el)"
                                    data-modelos="{{ strtolower($equipo->modelos->pluck('nombre')->implode(' ')) }}">
                                    <td class="px-5 text-xs text-gray-300 font-mono h-[52px] align-middle">
                                        {{ $loop->iteration }}</td>
                                    <td class="px-4 h-[52px] align-middle">
                                        <span
                                            class="inline-flex items-center font-mono text-xs font-semibold text-blue-dark bg-gray-100 px-2.5 py-1 rounded-lg">
                                            {{ $equipo->codigo }}
                                        </span>
                                    </td>
                                    <td class="px-4 h-[52px] align-middle">
                                        <span
                                            class="text-xs truncate block {{ $equipo->descripcion ? 'text-gray-500' : 'text-gray-300 italic' }}">
                                            {{ $equipo->descripcion ?? 'Sin descripción' }}
                                        </span>
                                    </td>
                                    <td class="px-4 h-[52px] align-middle">
                                        <div class="flex items-center gap-1 flex-wrap">
                                            @forelse($equipo->modelos as $mod)
                                                <span
                                                    class="inline-flex items-center px-2 py-0.5 rounded-md bg-blue/10 text-blue text-[10px] font-medium">
                                                    {{ $mod->nombre }}
                                                </span>
                                            @empty
                                                <span class="text-xs text-gray-300 italic">Sin modelos</span>
                                            @endforelse
                                        </div>
                                    </td>
                                    <td class="px-4 h-[52px] align-middle text-center">
                                        <span
                                            ...
                                            <span
                                                class="w-1.5 h-1.5 rounded-full {{ $equipo->activo ? 'bg-green' : 'bg-red' }}"></span>
                                    ...
                                    <td class="px-4 h-[52px] align-middle text-center">
                                        <div class="flex items-center justify-center gap-1">
                                            <button type="button"
                                                @click="abrirDrawer({{ $equipo->id }}, '{{ addslashes($equipo->codigo) }}', '{{ addslashes($equipo->descripcion ?? '') }}', {{ $equipo->activo ? 'true' : 'false' }}, {{ $equipo->modelos->pluck('id') }})"
                                        ...
                                        <form id="form-delete-eq-{{ $equipo->id }}" method="POST"
                                            action="{{ route('configuracion.equipos.destroy', $equipo) }}"
                                            class="hidden">
                                            @csrf @method('DELETE')
                                        </form>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-5 py-12 text-center">
                                        ...
                                        <p class="text-xs text-gray-300 mt-1">Usa el formulario de arriba para agregar
                                            el primero.</p>
                                    </td>
                                </tr>
                            @endforelse
                ...
                <div class="sm:hidden divide-y divide-gray-50">
                    @forelse($equipos as $equipo)
                        <div class="equipo-fila px-4 py-3.5"
                            x-show="filtraFila('{{ strtolower($equipo->codigo) }}', '{{ strtolower($equipo->descripcion ?? '') }}', {{ $equipo->activo ? 'true' : 'false' }}, $el)"
                            data-modelos="{{ strtolower($equipo->modelos->pluck('nombre')->implode(' ')) }}">
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 mb-1">
                                        <span
                                            class="font-mono text-xs font-bold text-blue-dark bg-gray-100 px-2 py-0.5 rounded-lg">
                                            {{ $equipo->codigo }}
                                        </span>
                                        <span
                                    ...
                                    @if ($equipo->descripcion)
                                        <p class="text-xs text-gray-500 mb-1.5 truncate">{{ $equipo->descripcion }}
                                        </p>
                                    @endif
                                    <div class="flex items-center gap-1 flex-wrap">
                                        @forelse($equipo->modelos as $mod)
                                        ...
                                        @empty
                                            <span class="text-[10px] text-gray-300 italic">Sin modelos</span>
                                        @endforelse
                                    </div>
                                </div>
                                <div class="flex items-center gap-1 flex-shrink-0">
                                    <button type="button"
                                        @click="abrirDrawer({{ $equipo->id }}, '{{ addslashes($equipo->codigo) }}', '{{ addslashes($equipo->descripcion ?? '') }}', {{ $equipo->activo ? 'true' : 'false' }}, {{ $equipo->modelos->pluck('id') }})"
                                        class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-blue hover:bg-blue/10 transition-all">
                                    ...
                                    <button type="button"
                                        @click="confirmarEliminar({{ $equipo->id }}, '{{ addslashes($equipo->codigo) }}')"
                                        class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-red hover:bg-red/10 transition-all">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                ...
                @if ($equipos->count() > 0)
                    <div
                        class="px-5 py-3 border-t border-gray-50 bg-gray-50/30 flex items-center justify-between min-h-[42px]">
        ...
        <div x-data="modelosManager()" x-init="init()" class="space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse($modelos as $modelo)
                                <tr class="modelo-fila transition-colors"
                                    ...
                                    :class="editandoId === {{ $modelo->id }} ?
                                        'bg-blue/[0.04] ring-1 ring-inset ring-blue/20' : 'hover:bg-gray-50/70'">
                                    <td class="px-5 text-xs text-gray-300 font-mono h-[52px] align-middle">
                                        {{ $loop->iteration }}</td>
                                    <td class="px-4 h-[52px] align-middle">
                                        <div class="relative h-[28px]">
                                            <span
                                                ...
                                                :class="editandoId === {{ $modelo->id }} ? 'opacity-0 pointer-events-none' :
                                                    'opacity-100'">
                                                <span
                                                    class="text-xs font-semibold text-blue-dark">{{ $modelo->nombre }}</span>
                                            </span>
                                            <input type="text" x-model="editNombre"
                                                ...
                                                :class="editandoId === {{ $modelo->id }} ? 'opacity-100' :
                                    ...
                                    <td class="px-4 h-[52px] align-middle">
                                        <div class="relative h-[28px]">
                                            <span
                                                class="absolute inset-0 flex items-center text-xs truncate transition-opacity duration-150 {{ $modelo->descripcion ? 'text-gray-500' : 'text-gray-300 italic' }}"
                                                :class="editandoId === {{ $modelo->id }} ? 'opacity-0 pointer-events-none' :
                                                    'opacity-100'">
                                                {{ $modelo->descripcion ?? 'Sin descripción' }}
                                            </span>
                                            <input type="text" x-model="editDescripcion"
                                    ...
                                    <td class="px-4 h-[52px] align-middle">
                                        <div class="relative h-[28px] flex items-center justify-center">
                                            <span
                                                class="absolute inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium border transition-opacity duration-150
                                                         {{ $modelo->activo ? 'bg-green/10 text-green border-green/30' : 'bg-red/10 text-red border-red/30' }}"
                                                :class="editandoId === {{ $modelo->id }} ? 'opacity-0 pointer-events-none' :
                                                    'opacity-100'">
                                                <span
                                                    class="w-1.5 h-1.5 rounded-full {{ $modelo->activo ? 'bg-green' : 'bg-red' }}"></span>
                                            ...
                                            <div class="absolute transition-opacity duration-150"
                                                ...
                                                <label class="inline-flex items-center cursor-pointer gap-2">
                                                    ...
                                                    <div
                                                        class="relative w-8 h-4 bg-gray-200 rounded-full transition-colors peer-checked:bg-blue
                                                                after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:after:translate-x-4">
                                                    </div>
                                                    <span class="text-xs text-gray-600 whitespace-nowrap"
                                                        x-text="editActivo ? 'Activo' : 'Inactivo'"></span>
                                                </label>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 h-[52px] align-middle">
                                        <div class="relative h-[28px] flex items-center justify-center">
                                            ...
                                            <form id="form-update-mod-{{ $modelo->id }}" method="POST"
                                                action="{{ route('configuracion.modelos.update', $modelo) }}"
                                                class="hidden">
                                                @csrf @method('PUT')
                                                <input type="text" name="nombre" x-bind:value="editNombre">
                                                <input type="text" name="descripcion"
                                                    x-bind:value="editDescripcion">
                                                <input type="hidden" name="activo"
                                                    x-bind:value="editActivo ? '1' : '0'">
                                            </form>
                                            <form id="form-delete-mod-{{ $modelo->id }}" method="POST"
                                                action="{{ route('configuracion.modelos.destroy', $modelo) }}"
                                                class="hidden">
                                                @csrf @method('DELETE')
                                            </form>
                                        </div>
                ...
                <div class="sm:hidden divide-y divide-gray-50">
                    @forelse($modelos as $modelo)
                        <div class="modelo-fila px-4 py-3.5"
                            x-show="filtraFila('{{ strtolower($modelo->nombre) }}', '{{ strtolower($modelo->descripcion ?? '') }}', {{ $modelo->activo ? 'true' : 'false' }})">
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1 min-w-0">
                                    {{-- Vista normal --}}
                                    <div :class="editandoId === {{ $modelo->id }} ? 'hidden' : 'block'">
                                        <div class="flex items-center gap-2 mb-0.5">
                                            <span
                                                class="text-xs font-bold text-blue-dark truncate">{{ $modelo->nombre }}</span>
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-medium border flex-shrink-0
                                                        {{ $modelo->activo ? 'bg-green/10 text-green border-green/30' : 'bg-red/10 text-red border-red/30' }}">
                                                <span
                                                    class="w-1.5 h-1.5 rounded-full {{ $modelo->activo ? 'bg-green' : 'bg-red' }}"></span>
                                                {{ $modelo->activo ? 'Activo' : 'Inactivo' }}
                                        ...
                                        @if ($modelo->descripcion)
                                            <p class="text-xs text-gray-400 truncate">{{ $modelo->descripcion }}</p>
                                        @endif
                                    </div>
                                    {{-- Vista edición --}}
                                    <div :class="editandoId === {{ $modelo->id }} ? 'block space-y-2' : 'hidden'">
                                        <input type="text" x-model="editNombre" placeholder="Nombre del modelo"
                                            class="w-full rounded-lg border border-blue bg-white px-2.5 py-1.5 text-xs font-semibold text-blue-dark focus:outline-none focus:shadow-[0_0_0_2px_rgba(59,130,246,0.25)]">
                                        <input type="text" x-model="editDescripcion"
                                            placeholder="Descripción opcional"
                                            class="w-full rounded-lg border border-blue/60 bg-white px-2.5 py-1.5 text-xs text-gray-700 focus:outline-none focus:border-blue focus:shadow-[0_0_0_2px_rgba(59,130,246,0.25)]">
                                        <label class="inline-flex items-center cursor-pointer gap-2">
                                            <input type="checkbox" x-model="editActivo" class="sr-only peer">
                                            <div
                                                class="relative w-8 h-4 bg-gray-200 rounded-full transition-colors peer-checked:bg-blue
                                ...
                                <div class="flex items-center gap-1 flex-shrink-0">
                                    {{-- Modo normal --}}
                                    <template x-if="editandoId !== {{ $modelo->id }}">
                                    ...
                                    <template x-if="editandoId === {{ $modelo->id }}">
                                        <div class="flex items-center gap-1">
                                            <button type="button" @click="guardarEdicion({{ $modelo->id }})"
                                                class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold text-white bg-green hover:bg-green-dark active:scale-95 transition-all">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2.5" d="M5 13l4 4L19 7" />
                            ...
                            {{-- Forms ocultos (solo necesarios en móvil también) --}}
                            <form id="form-update-mod-mob-{{ $modelo->id }}" method="POST"
                ...
                @if ($modelos->count() > 0)
                    <div
                        ...
                        <p x-show="editandoId !== null" class="text-xs text-blue flex items-center gap-1.5">
                            <kbd
                                class="px-1 py-0.5 bg-white border border-gray-200 rounded text-gray-500 font-mono text-[10px]">Enter</kbd>
                            guardar ·
                            <kbd
                                class="px-1 py-0.5 bg-white border border-gray-200 rounded text-gray-500 font-mono text-[10px]">Esc</kbd>
                            cancelar
                        </p>
                    </div>
                @endif
    ...
    ══════════════════════════════════════════════════════ --}}
    <div x-data="drawerEquipo()" x-init="init()" @abrir-drawer-equipo.window="abrir($event.detail)" x-cloak>

        {{-- Backdrop --}}
        <div x-show="open" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" @click="cerrar()"
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-300"
            ...
            class="fixed inset-y-0 right-0 w-full sm:w-[420px] bg-white shadow-2xl z-50 flex flex-col">

            {{-- Header --}}
            <div
                class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50/50 flex-shrink-0">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-orange/10 flex items-center justify-center">
                        <svg class="w-4 h-4 text-orange" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-blue-dark">Editar Equipo</h3>
                        <p class="text-xs text-gray-400 font-mono" x-text="equipo.codigo"></p>
                    </div>
                </div>
            ...
            <div class="flex-1 overflow-y-auto px-5 py-5 space-y-5">
                ...
                {{-- N° Serie --}}
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1.5">N° Serie / Código</label>
                    <input type="text" x-model="equipo.codigo"
                        class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-800 font-mono
                                  transition-all duration-200 focus:outline-none focus:border-orange focus:bg-white focus:shadow-[0_0_0_3px_rgba(255,140,66,0.15)]">
                </div>

                {{-- Descripción --}}
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1.5">Descripción <span
                            class="text-gray-400">(opcional)</span></label>
                    <input type="text" x-model="equipo.descripcion" placeholder="Descripción del equipo"
                        class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-800
                                  transition-all duration-200 focus:outline-none focus:border-orange focus:bg-white focus:shadow-[0_0_0_3px_rgba(255,140,66,0.15)]">
                </div>
                ...
                <div>
                    ...
                    <label class="inline-flex items-center cursor-pointer gap-2">
                        <input type="checkbox" x-model="equipo.activo" class="sr-only peer">
                        <div
                            class="relative w-8 h-4 bg-gray-200 rounded-full transition-colors peer-checked:bg-orange
                                    after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:after:translate-x-4">
                        </div>
                        <span class="text-xs text-gray-600 whitespace-nowrap"
                            x-text="equipo.activo ? 'Activo' : 'Inactivo'"></span>
                    </label>
                </div>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-2">
                        ...
                        <span class="ml-1.5 text-[10px] font-normal text-gray-400">
                            (<span x-text="equipo.modelosSeleccionados.length"></span> seleccionados)
                        </span>
                    </label>

                    {{-- Buscador modelos --}}
                    <div
                        class="flex items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 mb-2
                                focus-within:border-orange focus-within:bg-white focus-within:shadow-[0_0_0_3px_rgba(255,140,66,0.15)] transition-all">
                        <svg class="w-3.5 h-3.5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M21 21l-4.35-4.35M17 11A6 6 0 1 1 5 11a6 6 0 0 1 12 0z" />
                        </svg>
                        <input type="text" x-model="busquedaModelos" placeholder="Buscar modelo..."
                            class="w-full bg-transparent border-none p-0 text-xs text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-0">
                        <button x-show="busquedaModelos" @click="busquedaModelos = ''" type="button"
                            class="text-gray-400 hover:text-gray-600 transition-colors">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12" />
                    ...
                    {{-- Lista de modelos como checkboxes --}}
                    <div class="rounded-xl border border-gray-200 overflow-hidden divide-y divide-gray-50">
                        <template x-for="mod in modelosFiltrados" :key="mod.id">
                            <label class="flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors"
                                :class="equipo.modelosSeleccionados.includes(mod.id) ?
                                    'bg-blue/5 hover:bg-blue/8' :
                                    'bg-white hover:bg-gray-50'">
                                <div class="flex-shrink-0 w-4 h-4 rounded border-2 flex items-center justify-center transition-all duration-150"
                                    :class="equipo.modelosSeleccionados.includes(mod.id) ?
                                        'bg-blue border-blue' :
                                        'bg-white border-gray-300'">
                                    <svg x-show="equipo.modelosSeleccionados.includes(mod.id)"
                                    ...
                                    <input type="checkbox" class="sr-only"
                                        :checked="equipo.modelosSeleccionados.includes(mod.id)"
                                        @change="toggleModelo(mod.id)">
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-semibold truncate"
                                        :class="equipo.modelosSeleccionados.includes(mod.id) ? 'text-blue-dark' :
                                            'text-gray-700'"
                                        x-text="mod.nombre"></p>
                                    <p x-show="mod.descripcion" class="text-[10px] text-gray-400 truncate mt-0.5"
                                        x-text="mod.descripcion"></p>
                                </div>
                            </label>
                        </template>
                        <div x-show="modelosFiltrados.length === 0"
                            class="px-4 py-6 text-center text-xs text-gray-400 italic bg-white">
                            Sin resultados
                        </div>
                    </div>

                    {{-- Tags de seleccionados --}}
                    <div x-show="equipo.modelosSeleccionados.length > 0" class="mt-2 flex flex-wrap gap-1.5">
                        <template x-for="id in equipo.modelosSeleccionados" :key="id">
                            <span
                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue/10 text-blue text-[10px] font-semibold">
                                <span x-text="todosModelos.find(m => m.id === id)?.nombre ?? id"></span>
                                <button type="button" @click="toggleModelo(id)"
                                    class="text-blue/60 hover:text-blue transition-colors ml-0.5">
            ...
            {{-- Footer fijo --}}
            <div class="flex-shrink-0 px-5 py-4 border-t border-gray-100 bg-white flex items-center gap-3">
                ...
                <button type="button" @click="guardar()"
                    class="flex-1 px-4 py-2.5 text-sm font-semibold text-white bg-orange rounded-xl hover:bg-orange-dark active:scale-95 transition-all shadow-sm">
                    Guardar cambios
                </button>
            </div>

            {{-- URLs de update generadas por Laravel --}}
            <script>
                const EQUIPO_UPDATE_URLS = {
                    @foreach ($equipos as $equipo)
                        {{ $equipo->id }}: "{{ route('configuracion.equipos.update', $equipo) }}",
                    @endforeach
                };
            </script>
            <form id="form-drawer-equipo" method="POST" action="" class="hidden">
                @csrf @method('PUT')
                <input type="text" name="codigo" id="drawer-codigo">
                <input type="text" name="descripcion" id="drawer-descripcion">
                <input type="hidden" name="activo" id="drawer-activo">
                <div id="drawer-modelos-container"></div>
            </form>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm z-10 overflow-hidden"
            ...
            <div class="p-6">
                ...
                <div class="flex gap-2.5">
                    ...
                    <button
                        @click="document.getElementById('form-delete-' + (tipo === 'Equipo' ? 'eq' : 'mod') + '-' + id).submit()"
    ...
    <div class="hidden">
        <div
            class="bg-orange text-orange bg-orange/10 hover:bg-orange-dark border-orange/20 ring-orange/20 bg-orange/[0.04] peer-checked:bg-orange peer-checked:after:translate-x-4 translate-x-5">
        </div>
        ...
        <div
            class="bg-blue text-blue bg-blue/5 bg-blue/8 bg-blue/10 text-blue-dark bg-blue/[0.04] ring-blue/20 peer-checked:bg-blue border-blue">
        </div>
    </div>

    <script>
        // ─── Datos globales de modelos (para el drawer) ───────────────────────────
        const TODOS_MODELOS = @json($modelos->map(fn($m) => ['id' => $m->id, 'nombre' => $m->nombre, 'descripcion' => $m->descripcion ?? ''])->values());
        ...
        function equiposManager() {
            return {
                ...
                filtraFila(codigo, descripcion, activo, el) {
                    if (this.busqueda) {
                        const q = this.busqueda.toLowerCase().trim();
                        const modelos = el?.dataset?.modelos ?? '';
                        if (!codigo.includes(q) && !descripcion.includes(q) && !modelos.includes(q)) return false;
                    }
                ...
                contarResultados() {
                    this.$nextTick(() => {
                        this.resultadosFiltro = document.querySelectorAll(
                            '.equipo-fila:not([style*="display: none"])').length;
                    });
                },
                abrirDrawer(id, codigo, descripcion, activo, modelosIds) {
                    window.dispatchEvent(new CustomEvent('abrir-drawer-equipo', {
                        detail: {
                            id,
                            codigo,
                            descripcion,
                ...
                confirmarEliminar(id, codigo) {
                    window.dispatchEvent(new CustomEvent('confirmar-eliminar', {
                        detail: {
                            id,
                            nombre: codigo,
                            tipo: 'Equipo'
        ...
        function drawerEquipo() {
            return {
                ...
                get modelosFiltrados() {
                    const q = this.busquedaModelos.toLowerCase().trim();
                    if (!q) return this.todosModelos;
                    return this.todosModelos.filter(m =>
                ...
                init() {
                    document.addEventListener('keydown', (e) => {
                        if (this.open && e.key === 'Escape') this.cerrar();
                    });
                },
                ...
                abrir({
                    ...
                    this.equipo = {
                        ...
                        descripcion,
                        activo,
                        modelosSeleccionados: Array.isArray(modelosIds) ? modelosIds.map(Number) : [],
                    };
                    this.busquedaModelos = '';
                    this.open = true;
                ...
                toggleModelo(id) {
                    const idx = this.equipo.modelosSeleccionados.indexOf(id);
                    if (idx === -1) {
                        this.equipo.modelosSeleccionados.push(id);
                    } else {
                        this.equipo.modelosSeleccionados.splice(idx, 1);
                    }
                },

                guardar() {
                    const url = EQUIPO_UPDATE_URLS[this.equipo.id];
                    if (!url) {
                        console.error('URL no encontrada para equipo', this.equipo.id);
                        return;
                    }

                    const form = document.getElementById('form-drawer-equipo');
                    form.action = url;
                    document.getElementById('drawer-codigo').value = this.equipo.codigo;
                    document.getElementById('drawer-descripcion').value = this.equipo.descripcion ?? '';
                    document.getElementById('drawer-activo').value = this.equipo.activo ? '1' : '0';

                    const container = document.getElementById('drawer-modelos-container');
                    container.innerHTML = '';
                    if (this.equipo.modelosSeleccionados.length === 0) {
                        const inp = document.createElement('input');
                        inp.type = 'hidden';
                        inp.name = 'modelos_ids';
                        inp.value = '';
                        container.appendChild(inp);
                    } else {
                        this.equipo.modelosSeleccionados.forEach(id => {
                            const inp = document.createElement('input');
                            inp.type = 'hidden';
                            inp.name = 'modelos_ids[]';
                            inp.value = id;
            ...
            return {
                ...
                init() {
                    ...
                    document.addEventListener('keydown', (e) => {
                        ...
                        if (e.key === 'Enter') {
                            e.preventDefault();
                            this.guardarEdicion(this.editandoId);
                        }
                        if (e.key === 'Escape') {
                            e.preventDefault();
                ...
                contarResultados() {
                    this.$nextTick(() => {
                        this.resultadosFiltro = document.querySelectorAll(
                            '.modelo-fila:not([style*="display: none"])').length;
                    });
                },
                iniciarEdicion(id, nombre, descripcion, activo) {
                    ...
                    this.editActivo = activo;
                },
                cancelarEdicion() {
                    this.editandoId = null;
                    this.editNombre = this.editDescripcion = '';
                    this.editActivo = true;
                },
                guardarEdicion(id) {
                    const isMobile = window.innerWidth < 640;
                    const formId = isMobile ? 'form-update-mod-mob-' + id : 'form-update-mod-' + id;
