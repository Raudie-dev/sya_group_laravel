<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-gray-400 text-sm mt-0.5">Gestión de códigos de equipos</p>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <div class="p-4 sm:p-6 lg:p-8 space-y-5"
        ...
        <div class="grid grid-cols-3 gap-3">
            @php
                $total     = $equipos->count();
                $activos   = $equipos->where('activo', true)->count();
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm px-4 py-3.5 flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-bold text-blue-dark leading-none">{{ $total }}</p>
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm px-4 py-3.5 flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-bold text-blue-dark leading-none">{{ $activos }}</p>
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm px-4 py-3.5 flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-bold text-blue-dark leading-none">{{ $inactivos }}</p>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <div class="flex-1">
                            <label class="block text-xs font-medium text-gray-500 mb-1.5">
                                Código del Equipo <span class="text-orange">*</span>
                            </label>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center rounded-xl border border-gray-200 bg-white overflow-hidden divide-x divide-gray-200 text-xs font-medium">
                        <button type="button" @click="filtroEstado = 'todos'"
                                ...
                                :class="filtroEstado === 'todos' ? 'bg-orange text-white' : 'text-gray-500 hover:bg-gray-50'">
                            Todos
                        </button>
                        <button type="button" @click="filtroEstado = 'activos'"
                                class="px-3 py-2 transition-colors"
                                :class="filtroEstado === 'activos' ? 'bg-green text-white' : 'text-gray-500 hover:bg-gray-50'">
                            Activos
                        </button>
                        <button type="button" @click="filtroEstado = 'inactivos'"
                                class="px-3 py-2 transition-colors"
                                :class="filtroEstado === 'inactivos' ? 'bg-red text-white' : 'text-gray-500 hover:bg-gray-50'">
                            Inactivos
                        </button>
                    </div>
            ...
            {{-- ═══════════════════════════════════════════════════════════
                 TABLA con table-fixed + colgroup para anchos fijos.
                 Cada celda editable usa position:relative con span e input
                 superpuestos (absolute), así la fila NUNCA cambia de altura.
            ═══════════════════════════════════════════════════════════ --}}
            <div class="overflow-x-auto">
                <table class="w-full text-sm table-fixed">
                    <colgroup>
                        <col style="width:40px">       {{-- # --}}
                        <col style="width:160px">      {{-- Código --}}
                        <col>                           {{-- Descripción --}}
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($equipos as $equipo)
                            <tr class="equipo-fila transition-colors"
                                ...
                                :class="editandoId === {{ $equipo->id }}
                                    ? 'bg-orange/[0.04] ring-1 ring-inset ring-orange/20'
                                    : 'hover:bg-gray-50/70'">
                                ...
                                {{-- ── CÓDIGO
                                     El contenedor tiene altura fija (h-[28px]) y position:relative.
                                     Span e input se solapan en absolute, cambiando solo la opacidad.
                                     → La fila no se mueve ni un píxel al editar. --}}
                                <td class="px-4 h-[52px] align-middle">
                                    <div class="relative h-[28px]">
                                        ...
                                        <input type="text"
                                               x-model="editCodigo"
                                               class="absolute inset-0 w-full rounded-lg border border-orange bg-white px-2.5 text-xs font-mono
                                                      font-semibold text-blue-dark transition-opacity duration-150
                                                      focus:outline-none focus:shadow-[0_0_0_2px_rgba(255,140,66,0.25)]"
                                ...
                                <td class="px-4 h-[52px] align-middle">
                                    <div class="relative h-[28px]">
                                        {{-- Lectura --}}
                                        <span class="absolute inset-0 flex items-center text-xs truncate transition-opacity duration-150
                                                     {{ $equipo->descripcion ? 'text-gray-500' : 'text-gray-300 italic' }}"
                                              :class="editandoId === {{ $equipo->id }} ? 'opacity-0 pointer-events-none' : 'opacity-100'">
                                            {{ $equipo->descripcion ?? 'Sin descripción' }}
                                        </span>
                                        {{-- Edición --}}
                                        <input type="text"
                                               x-model="editDescripcion"
                                               placeholder="Descripción opcional"
                                               class="absolute inset-0 w-full rounded-lg border border-orange/60 bg-white px-2.5 text-xs text-gray-700
                                                      transition-opacity duration-150
                                                      focus:outline-none focus:border-orange focus:shadow-[0_0_0_2px_rgba(255,140,66,0.25)]"
                                               :class="editandoId === {{ $equipo->id }} ? 'opacity-100' : 'opacity-0 pointer-events-none'">
                                    </div>
                                </td>
                                ...
                                <td class="px-4 h-[52px] align-middle">
                                    <div class="relative h-[28px] flex items-center justify-center">
                                        {{-- Lectura (clases resueltas en PHP, sin binding) --}}
                                        <span class="absolute inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium border
                                                     transition-opacity duration-150
                                                     {{ $equipo->activo
                                ...
                                <td class="px-4 h-[52px] align-middle">
                                    <div class="relative h-[28px] flex items-center justify-center">
                                        ...
                                        <div class="absolute flex items-center gap-1.5 transition-opacity duration-150"
                                             :class="editandoId === {{ $equipo->id }} ? 'opacity-100' : 'opacity-0 pointer-events-none'">
                                            <button type="button"
                                                    @click="guardarEdicion({{ $equipo->id }})"
                                                    class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold
                                                           text-white bg-green hover:bg-green-dark active:scale-95 transition-all duration-150">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        {{-- Forms ocultos --}}
                                        <form id="form-update-{{ $equipo->id }}" method="POST"
                                        ...
                                        <form id="form-delete-{{ $equipo->id }}" method="POST"
                                ...
                                <td colspan="5" class="px-5 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-400 mt-1">Usa el formulario de arriba para agregar el primero.</p>
                                        </div>
                                    </div>
                                </td>
                            </tr>
            ...
            {{-- Sin resultados --}}
            <div x-show="(busqueda || filtroEstado !== 'todos') && resultadosFiltro === 0"
                 class="px-5 py-8 text-center border-t border-gray-50">
                <p class="text-xs text-gray-400">
                    Sin resultados
                    <span x-show="busqueda"> para <span x-text="'«' + busqueda + '»'" class="font-semibold text-gray-500"></span></span>
                    <span x-show="filtroEstado !== 'todos'"> · filtro <span x-text="filtroEstado" class="font-semibold text-gray-500"></span></span>
                </p>
            </div>
            ...
            @if($equipos->count() > 0)
                <div class="px-5 py-3 border-t border-gray-50 bg-gray-50/30 flex items-center justify-between min-h-[42px]">
                    ...
                    <p x-show="editandoId !== null"
                       x-transition:enter="transition ease-out duration-150"
                       x-transition:enter-start="opacity-0 translate-x-2"
                       x-transition:enter-end="opacity-100 translate-x-0"
                       class="text-xs text-orange flex items-center gap-1.5">
                        <svg class="w-3 h-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    {{-- ── MODAL ELIMINACIÓN ── --}}
    <div x-data="{ open: false, id: null, codigo: '' }"
         @confirmar-eliminar.window="id = $event.detail.id; codigo = $event.detail.codigo; open = true"
         x-show="open" x-cloak
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm z-10 overflow-hidden"
            ...
            <div class="p-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-blue-dark">Eliminar Equipo</h3>
                        <p class="text-xs text-gray-400 mt-0.5">Esta acción no se puede deshacer</p>
                    </div>
                </div>
                <p class="text-sm text-gray-600 mb-5">
                    ¿Seguro que deseas eliminar
                    <span class="font-mono font-semibold text-blue-dark" x-text="'«' + codigo + '»'"></span>?
                </p>
                <div class="flex gap-2.5">
                    ...
                    <button @click="document.getElementById('form-delete-' + id).submit()"
    ...
    {{-- Safelist --}}
    <div class="hidden">
        <div class="bg-orange text-orange bg-orange/10 hover:bg-orange-dark border-orange/20 ring-orange/20 bg-orange/[0.04]"></div>
        ...
        <div class="bg-blue text-blue bg-blue/10 text-blue-dark"></div>
        <div class="peer-checked:bg-orange peer-checked:after:translate-x-4"></div>
    </div>
    ...
    function equiposManager() {
        return {
            ...
            contarResultados() {
                this.$nextTick(() => {
                    let v = 0;
                    document.querySelectorAll('.equipo-fila').forEach(f => {
                        if (f.style.display !== 'none') v++;
                    });
                    this.resultadosFiltro = v;
                });
            },
            ...
            cancelarEdicion() {
                ...
                this.editCodigo = this.editDescripcion = '';
            ...
            confirmarEliminar(id, codigo) {
                window.dispatchEvent(new CustomEvent('confirmar-eliminar', { detail: { id, codigo } }));
            },
        };
    }
    </script>
